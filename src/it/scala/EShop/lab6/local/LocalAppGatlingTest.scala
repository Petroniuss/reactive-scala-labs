...
class LocalAppGatlingTest extends Simulation {
  ...
  val scn = scenario("BasicSimulation")
    .feed(jsonFile(classOf[LocalAppGatlingTest].getResource("/data/search_data.json").getPath).random)
    .exec(
      http("search")
  ...
  setUp(
    scn.inject(rampUsers(10000).during(1.minute))
