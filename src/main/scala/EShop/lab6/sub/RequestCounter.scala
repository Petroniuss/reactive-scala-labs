...
object RequestCounter {
  val RequestCounterServiceKey = ServiceKey[RequestCounterCommand]("RequestCounter")
  ...
  def apply(): Behavior[RequestCounterCommand] = Behaviors.setup { context =>
    ...
    val adapter = context.messageAdapter[RequestCounterTopicMessage] {
      case ProductsEndpointHitMessage => ProductsEndpointHit
    }
  ...
  def countRequests(state: Int, log: Logger): Behavior[RequestCounterCommand] = Behaviors.receiveMessage {
