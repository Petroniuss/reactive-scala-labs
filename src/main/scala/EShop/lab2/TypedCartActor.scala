...
object TypedCartActor {
  ...
  def apply(orderManagerCartListener: ActorRef[TypedCartActor.Event],
            orderManagerCheckoutListener: ActorRef[TypedCheckout.Event],
            orderManagerPaymentListener: ActorRef[Payment.Event]): Behavior[Command] =
    Behaviors.withTimers(timers =>
      new TypedCartActor(timers,
        orderManagerCartListener,
        orderManagerCheckoutListener,
        orderManagerPaymentListener).start
    )
}
...
class TypedCartActor(timers: TimerScheduler[Command],
  ...
  def empty: Behavior[Command] = Behaviors.receive((context, msg) =>
    msg match {
      case AddItem(item) =>
        scheduleExpireCart()
  ...
  def nonEmpty(cart: Cart): Behavior[Command] = Behaviors.receive((context, msg) =>
    msg match {
      case AddItem(item) =>
        scheduleExpireCart()
      ...
      case StartCheckout =>
        cancelExpireCartTimer()
        val checkoutBehavior = TypedCheckout(
          context.self,
          orderManagerCheckoutListener,
  ...
  def inCheckout(cart: Cart): Behavior[Command] = Behaviors.receive((context, msg) =>
    msg match {
